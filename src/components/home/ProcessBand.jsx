  ...
  return (
    <li className="relative flex flex-col items-center px-0.5 text-center sm:px-2">
      ...
      */}
      ...
      {/* 11px on a phone. Five steps share 358px of content across four 4px
          gaps, so each is 68px — a 48px mark and a two-line title fit, and
          14px type would put one word on each line. */}
      <h3 className="mt-3 text-[0.6875rem] font-semibold leading-tight text-ink sm:mt-4 sm:text-sm sm:leading-normal">
        {step.title}
      </h3>
      ...
      {/* A size down from the title, as the reference sets it — at the same
          ...
          Hidden below `sm`, which is how the mobile reference fits five
          across: "Finest tea leaves hand picked" in 68px is four lines of 9px
          type, so the reference shows the titles alone and so does this. */}
      {step.description ? (
        <p className="mt-1 hidden text-xs leading-relaxed text-balance text-ink-muted sm:block">
          {step.description}
        </p>
      ) : null}
    </li>
      ...
      */}
      <div className="relative mx-auto max-w-(--container-max) pl-(--gutter-l) pr-(--gutter-r)">
        <div className="lg:max-w-[calc(100%-18rem)] xl:max-w-[calc(100%-24rem)]">
          ...
          {/* One line on a phone. "Our Premium Tea Range" measures 227px at
              20px, so this 23-character heading is about 249px — inside the
              288px a 320 screen has. At 30px it is ~373px, which is why it
              broke after "Your". */}
          <h2 id="process-heading" className="mt-2 font-display text-xl font-bold text-ink sm:text-4xl">
          ...
          {/* Five across at every width, as the mobile reference draws them.
              358px over five steps and four 4px gaps is 68px each, which is
              why the titles come down to 11px and the descriptions wait for
              `sm`. */}
          <ol className="mt-8 grid grid-cols-5 gap-x-1 sm:mt-10 sm:gap-x-6 lg:gap-x-4 xl:gap-x-6">
